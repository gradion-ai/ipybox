...
In addition to `stdio` based MCP servers that run inside the container, `ipybox` also supports connecting to external MCP servers running with `streamable-http` or legacy `sse` transports. This enables integration with MCP servers running as separate services.

Start the [test MCP server](https://github.com/gradion-ai/ipybox/blob/main/tests/mcp_server.py) in a separate terminal on the host machine:
