...
Run the Python tool with `execute_ipython_cell` tool of the `ipybox` MCP server using 2-3 example inputs to observe return value patterns:
...
Call the `reset` tool of the `ipybox` MCP server to restart the IPython kernel so the next import loads the modified module.

Then test with `execute_ipython_cell` using the same example inputs from step 1:
